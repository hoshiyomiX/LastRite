...
// Removed modular import that was causing issues
// import { htmlContent } from './pages/generator.js'; 

// SAFE HTML GENERATOR FUNCTION
// Uses concatenation to avoid parser ambiguity with template literals
function getGeneratorPage() {
  var css = ":root { --primary: #00f2ea; --secondary: #ff0050; --bg: #0a0a0a; --surface: #161616; --text: #ffffff; --muted: #888888; } " +
  ...
  var script = "document.getElementById('domain').value = window.location.hostname; " +
    "function generateLink() { " +
    "  const domain = document.getElementById('domain').value; " +
    "  const format = document.getElementById('format').value; " +
    "  const origin = window.location.origin; " +
    "  let finalUrl = ''; " +
    "  if (format === 'clash') { finalUrl = origin + '/sub?host=' + domain + '&format=clash'; } " +
    "  else if (format === 'v2ray') { finalUrl = origin + '/api/v1/sub?host=' + domain + '&format=v2ray'; } " +
    "  else { finalUrl = origin + '/api/v1/sub?host=' + domain + '&format=raw'; } " +
  ...
  return '<!DOCTYPE html>' +
    ...
    '<div class="form-group"><label>Domain / SNI</label><input type="text" id="domain" placeholder="example.com" value=""></div>' +
    '<div class="form-group"><label>Format</label><select id="format"><option value="raw">Raw (Clash/Meta)</option><option value="v2ray">V2Ray (Base64)</option><option value="clash">Clash Provider</option></select></div>' +
    '<button onclick="generateLink()">Generate Link</button>' +
    '<div id="result"><button class="copy-btn" onclick="copyToClipboard()">Copy</button><code id="output"></code></div>' +
    '</div>' +
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/" || url.pathname === "/sub") {
        return new Response(getGeneratorPage(), { 
          headers: {
